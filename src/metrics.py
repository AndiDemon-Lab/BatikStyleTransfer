"""
Comprehensive metrics module for Neural Style Transfer evaluation.
Implements multiple image quality assessment metrics for NST research.
"""
...
class LossTracker:
    ...
    def add_loss(self, epoch, content_loss, style_loss, total_loss):
        ...
        self.content_losses.append({
            'epoch': epoch,
            'value': float(content_loss)
        })
        self.style_losses.append({
            'epoch': epoch,
            'value': float(style_loss)
        })
        self.total_losses.append({
            'epoch': epoch,
